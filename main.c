...
void generate_texts(array_t token_graph, array_t tokens);
...
int main(void)
{
	...
	generate_texts(token_graph, tokens);
...
{
	printf("\n");
	char *phrase_initial_words[] = {"If", "", ""};
	generate_phrase(token_graph, phrase_initial_words, tokens);
	printf("\n");
	generate_phrase(token_graph, phrase_initial_words, tokens);
	printf("\n");
	generate_phrase(token_graph, phrase_initial_words, tokens);
...
node_t* find_word(char** word, array_t word_graph, array_t tokens)
{
	...
	for(i = 0; i < word_graph.length; i++)
	{
		...
		if(strcmp(current_word, word[0]) == 0)
		{
			return element;
		}
	}
...
node_t* word_append_to_graph(array_t* word_graph, long *index)
{
	...
	{
		word_node.token[param_counter] = index[param_counter];
	}
